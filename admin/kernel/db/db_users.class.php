...
*/
	...
	public function set_blacklist()
	{
		...
		if(empty($node))
		{
			...
			error_log('platonicles: Blacklist - New IP added - '.$ip);
		}
		else
		{
			$date = $node[0]->getChild('date');
			...
			if($current_time > $date + (BLACKLIST_TIME*60))
			{
				...
				error_log('platonicles: Blacklist - IP renewed because is expired - '.$ip);
			}
			else
			{
				$fail_count += 1;
				...
				error_log('platonicles: Blacklist - IP fail count('.$fail_count.') - '.$ip);
			}
		}
