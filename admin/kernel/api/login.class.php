...
class Login {
	...
	*/
	public function logout()
	{
		...
		// Clean remember me
		setcookie('platonicles_hash', '', time()-42000);
		setcookie('platonicles_id', '', time()-42000);
	}
	...
	*/
	public function remember_me()
	{
		...
		// Check cookies
		if( !isset($_COOKIE['platonicles_hash']) || !isset($_COOKIE['platonicles_id']) )
			return false;

		// Sanitize cookies
		$cookie_hash	= Validation::sanitize_html($_COOKIE['platonicles_hash']);
		$cookie_id		= Validation::sanitize_int($_COOKIE['platonicles_id']);
	...
	*/
	public function set_remember_me()
	{
		...
		// Set cookies
		setcookie('platonicles_hash', $tmp_hash, time()+(3600*24*15));
		setcookie('platonicles_id', $this->get_user_id(), time()+(3600*24*15));
...
*/
	...
	*/
	private function get_key()
	{
		...
		if(empty($agent))
			$agent = 'platonicles/4.0 (Mr Nibbler Protocol)';
